<?php
    ...
    $sql="UPDATE `notifications` SET `title` =?, `description` = ?, `img` = ? WHERE `id` = ?;";
    ...
    if($result->execute()){
        ...
        header("location:update-notification.php?ok=10&id=".$_POST["news_id"]);

    }
    else{
        header("location:update-notification.php?error=11&id=".$_POST["news_id"]);
    }
